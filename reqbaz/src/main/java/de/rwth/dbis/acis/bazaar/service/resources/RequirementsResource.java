...
public class RequirementsResource {
    ...
    public Response getRequirementsForProject(int projectId, int page, int perPage, String search, String stateFilter, List<String> sort, String sortDirection) {
        ...
        try {
            ...
            if (dalFacade.isProjectPublic(projectId)) {
                boolean authorized = new AuthorizationManager().isAuthorized(internalUserId, PrivilegeEnum.Read_PUBLIC_REQUIREMENT, projectId, dalFacade);
                if (!authorized) {
                ...
                boolean authorized = new AuthorizationManager().isAuthorized(internalUserId, PrivilegeEnum.Read_REQUIREMENT, projectId, dalFacade);
                if (!authorized) {
    ...
    public Response getRequirementsForCategory(int categoryId, int page, int perPage, String search, String stateFilter, List<String> sort, String sortDirection) {
        ...
        try {
            ...
            if (dalFacade.isCategoryPublic(categoryId)) {
                boolean authorized = new AuthorizationManager().isAuthorized(internalUserId, PrivilegeEnum.Read_PUBLIC_REQUIREMENT, project.getId(), dalFacade);
                if (!authorized) {
                    ExceptionHandler.getInstance().throwException(ExceptionLocation.BAZAARSERVICE, ErrorCode.AUTHORIZATION, Localization.getInstance().getResourceBundle().getString("error.authorization.anonymous"));
                }
            } else {
                boolean authorized = new AuthorizationManager().isAuthorized(internalUserId, PrivilegeEnum.Read_REQUIREMENT, project.getId(), dalFacade);
                if (!authorized) {
                    ExceptionHandler.getInstance().throwException(ExceptionLocation.BAZAARSERVICE, ErrorCode.AUTHORIZATION, Localization.getInstance().getResourceBundle().getString("error.authorization.category.read"));
                }
            }
    ...
    public Response getRequirement(@PathParam("requirementId") int requirementId) {
        ...
        try {
            ...
            if (dalFacade.isRequirementPublic(requirementId)) {
                ...
                boolean authorized = new AuthorizationManager().isAuthorized(internalUserId, PrivilegeEnum.Read_REQUIREMENT, requirement.getProjectId(), dalFacade);
                if (!authorized) {
                    ExceptionHandler.getInstance().throwException(ExceptionLocation.BAZAARSERVICE, ErrorCode.AUTHORIZATION, Localization.getInstance().getResourceBundle().getString("error.authorization.category.read"));
                }
            }
    ...
    public Response updateRequirement(@ApiParam(value = "Requirement entity", required = true) Requirement requirementToUpdate) {
        ...
        try {
            ...
            if (bex.getErrorCode() == ErrorCode.AUTHORIZATION) {
                ...
                logger.warning(bex.getMessage());
                Context.get().monitorEvent(MonitoringEvent.SERVICE_ERROR, "Update requirement " + requirementToUpdate.getId());
                return Response.status(Response.Status.INTERNAL_SERVER_ERROR).entity(ExceptionHandler.getInstance().toJSON(bex)).build();
            }
        } catch (Exception ex) {
            BazaarException bex = ExceptionHandler.getInstance().convert(ex, ExceptionLocation.BAZAARSERVICE, ErrorCode.UNKNOWN, ex.getMessage());
    ...
    public Response developRequirement(@PathParam("requirementId") int requirementId) {
        ...
        try {
            ...
            Integer internalUserId = dalFacade.getUserIdByLAS2PeerId(userId);
            Requirement requirement = dalFacade.getRequirementById(requirementId, internalUserId);
            boolean authorized = new AuthorizationManager().isAuthorized(internalUserId, PrivilegeEnum.Create_DEVELOP, requirement.getProjectId(), dalFacade);
            if (!authorized) {
    ...
    public Response undevelopRequirement(@PathParam("requirementId") int requirementId) {
        ...
        try {
            ...
            Integer internalUserId = dalFacade.getUserIdByLAS2PeerId(userId);
            Requirement requirement = dalFacade.getRequirementById(requirementId, internalUserId);
            boolean authorized = new AuthorizationManager().isAuthorized(internalUserId, PrivilegeEnum.Delete_DEVELOP, requirement.getProjectId(), dalFacade);
            if (!authorized) {
